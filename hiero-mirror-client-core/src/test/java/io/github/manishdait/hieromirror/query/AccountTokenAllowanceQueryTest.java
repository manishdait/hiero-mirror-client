...
public class AccountTokenAllowanceQueryTest {
  ...
  void shouldCreateQueryWithDefaults() {
    var query = new AccountTokenAllowanceQuery();
  ...
  void shouldSetAccountId() {
    var query = new AccountTokenAllowanceQuery().setAccountId(ID);
  ...
  void shouldSetEvmAddress() {
    ...
    var query = new AccountTokenAllowanceQuery().setEvmAddress(evmAddress);
  ...
  void shouldSetAlias() {
    ...
    var query = new AccountTokenAllowanceQuery().setAlias(alias);
  ...
  @Test
  void shouldSetOrder() {
    var query = new AccountTokenAllowanceQuery().setOrder(ORDER);
  ...
  @Test
  void shouldSetLimit() {
    var query = new AccountTokenAllowanceQuery().setLimit(LIMIT);
  ...
  void shouldRaiseErrorWhenLimitIsGreaterThan100() {
    Assertions.assertThatThrownBy(() -> new AccountTokenAllowanceQuery().setLimit(101))
  ...
  void shouldRaiseErrorWhenLimitIsLessThan1() {
    Assertions.assertThatThrownBy(() -> new AccountTokenAllowanceQuery().setLimit(0))
        .isInstanceOf(IllegalArgumentException.class)
        .hasMessage("limit must be greater than 0 and less than 100");
  }
  ...
  void shouldSetTokenId() {
    var query =
        new AccountTokenAllowanceQuery().setTokenId(TOKEN_ID.getOperator(), TOKEN_ID.getValue());
  ...
  void shouldSetSpenderId() {
    var query =
        new AccountTokenAllowanceQuery()
  ...
  void shouldBuildRequestWithAllParams() {
    ...
    Assertions.assertThat(request.getUrl())
        .isEqualTo("https://example.com/api/v1/accounts/" + ID.toString() + "/allowances/tokens");
    Assertions.assertThat(request.getMethod()).isEqualTo("GET");

    Assertions.assertThat(request.getQueryParams())
        .extractingByKeys("limit", "order", "token.id", "spender.id")
        .contains(
  ...
  void shouldRaiseErrorOnBuildRequestIfNoIdentifierSet() {
    ...
    var query = new AccountTokenAllowanceQuery().setLimit(LIMIT).setOrder(ORDER);
